...
program
  .command('transition')
  .description(
    'Transition Linear issue(s) based on a GitHub event. Ticket refs are optional — no-op if none found.'
  )
  .requiredOption(
    '--event-type <type>',
  ...
  .option(
    '--review-state <state>',
    'Review state (approved | changes_requested) — required for PR_REVIEW_SUBMITTED'
  )
  ...
  .option(
    '--linear-key <key>',
    'Explicit Linear issue key override (e.g. ELY-5), bypasses extraction'
  )
  .option(
    '--preview-url <url>',
    'Vercel preview URL — posted as a comment when ticket moves to In Progress'
  )
